<script lang="ts">
	...
	let phaseHeading = $state.raw<HTMLHeadingElement>();
	let confirmedIdentity = $state('');
	...
	$effect(() => {
		const currentIdentity = status === 'received' ? (receivedIdentityLabel ?? '') : '';
		if (confirmedIdentity === currentIdentity) return;
...
<main class="link-target-stage">
	<section class="link-target-card">
		...
		{#if status === 'waiting'}
			...
			<h1 bind:this={phaseHeading} tabindex="-1">Scan with your trusted device</h1>
			<p class="muted">
				In AURA on the device that already has your profile, open Profile → Link another device.
			</p>
			...
			<label class="link-manual-url">
				<span>Full device-link URL</span>
				<textarea
					readonly
					rows="3"
					value={linkUrl}
					onfocus={(event) => event.currentTarget.select()}
					aria-label="Full device-link URL"></textarea>
			</label>
		{:else if status === 'received'}
			<div class="status-icon"><ShieldCheck size={24} /></div>
			<p class="eyebrow">Encrypted profile received</p>
			<h1 bind:this={phaseHeading} tabindex="-1">Protect it on this device</h1>
			<div class="link-profile-summary target">
				...
				<div>
					...
					<p class="mono muted">{receivedIdentityLabel}</p>
				</div>
			</div>
			<p class="muted">
				Confirm the full npub above belongs to your existing profile before continuing.
			</p>
			<label class="link-identity-confirmation">
				<input type="checkbox" bind:checked={identityConfirmed} disabled={busy} />
				<span>I verified this full npub on my existing device.</span>
			</label>
			<div class="link-code-block target">
			...
			<p class="muted">
				Your device will create a new local Passkey envelope. The source Passkey is not copied.
			</p>
			{#if error}<div class="inline-error" role="alert">{error}</div>{/if}
			<button
		...
		{:else}
			...
			<h1 bind:this={phaseHeading} tabindex="-1">
				{status === 'expired' ? 'This QR expired' : 'Linking could not start'}
			</h1>
